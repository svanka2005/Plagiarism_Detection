...
def containment(ngram_array):
    '''Calculates containmnet values for the given dataframe.
       :param df: an array of ngram counts for an answer anbd source text
       :param ngram_a: answer text ngarm count
       :return: a normalized containment vcalue '''
    
    #print(ngram_a)
    answer_text = ngram_array[0] # answer text
    source_text = ngram_array[1] # source text
    
    #number_of_equal_elements = np.sum(answer_text==source_text) #intersection
    
    number_of_equal_elements = sum(min(answer_text,source_text) for answer_text , source_text in zip(*ngram_array))
    
    containment_val = float(number_of_equal_elements) / float(ngram_array[0].sum())
